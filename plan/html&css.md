...
### 课前提问：什么事前段开发，前段开发的工作内容包括什么，做前端开发需要学习哪些基础知识。
#### 课前调查
...
*练习*：制作一个没有样式的网页示例 [示例链接](example/1-html标签入门.html)



当这里，同学们已经完成了一个有内容的网页。但是同学们有没有发现一个问题，哪些元素自己占用了一行，哪些元素会和其他元素保持在一行。

#### 块元素与行级元素的区别
行级元素：h p ul li div
块元素：a img
区别：块元素独立成行，行级元素可以不独立成行。

那么有没有办法把两个块元素放到一行显示呢，肯定是可以的，具体方法，在下一节css中会讲解。
...
*练习:*
1. 照着网页实例，画出树状结构，并用div模拟这个网页的结构。
2. 在各个结构的div中填充内容。
...
*第三天结束，以练习为主，主要目的是巩固前两天的html，css基础知识和布局知识*
